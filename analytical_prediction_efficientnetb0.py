...
def cifar_convert_to_layers_mixed(block_info,quant_list,cifar=True):
    if cifar:
        ...
        output_dim=[112]+[56]*2+[28]*2+[28]*3+[14]*3+[7]*4+[7]
        num_layer_list=(1+2+2+3+3+4+1)*[1]
        num_channel_list=[16]+[24]*2+[48]*2+[80]*3+[128]*3+[192]*4+[320]
        stride_list=[1,2,1, 2,1, 1,1,1, 2,1,1, 2,1,1,1, 1]
...
def worker(id):   
    ...
    quant_options=[8]
    block_info_test=['k3e1']+['k3e6']*2+['k5e6']*2+['k3e6']*3+['k5e6']*3+['k5e6']*4+['k3e6']
