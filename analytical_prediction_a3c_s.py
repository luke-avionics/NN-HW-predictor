...
def cifar_convert_to_layers_mixed(block_info,quant_list,cifar=True):
    if cifar:
        ...
        output_dim=[42]*4+[21]*4+[10]*4
        num_layer_list=(12)*[1]
        num_channel_list=[16]*4+[32]*4+[64]*4
        stride_list=[1,1,1,1, 2,1,1,1, 2,1,1,1]
    ...
    layer_wise_quant=[]
    layer_block_corr={}
    for i in range(sum(num_layer_list)+2):
        layer_block_corr[i]=[]
    layer_num=0
    net_struct.append([4,16,42,3,2])
    dw+=[False]
        ...
        if "g" not in block_info[i] and block_info[i] != "skip" and 'c' not in block_info[i]:
            ...
            if num_layer_list[i]==1:
                if i==0:
                    ...
                    quant_bit=quant_list.pop(0)
                    layer_wise_quant+=[quant_bit[0],quant_bit[1],quant_bit[0]]
                    layer_block_corr[i+1]+=[layer_num,layer_num+1,layer_num+2]
                    layer_num+=3
                ...
                if i==0:
                    ...
                    quant_bit=quant_list.pop(0)
                    layer_wise_quant+=[quant_bit[0]]
                    layer_block_corr[i+1]+=[layer_num]
                    layer_num+=1
    ...
    net_struct.append([64,64,10,1,1])
    dw+=[False]
...
def allocate_layers_rl(net_struct,quant_list,dw,platform_specs,layer_block_corr,cifar=True,edd=False,channel_part=False):
    ...
    if not channel_part:
        if cifar:    
            ...
            for i, quant_bit in enumerate(std_quantization_bins.keys()):
                for layer in std_quantization_bins[quant_bit]:
                    if net_struct[layer][2]==42:
                        if "a0"+"q"+str(quant_bit) not in accelerator_types:
                            accelerator_types.append("a0"+"q"+str(quant_bit))
                        accelerator_alloc[layer]="a0"+"q"+str(quant_bit)
                    elif net_struct[layer][2]==21:
                        if "a1"+"q"+str(quant_bit) not in accelerator_types:
                            accelerator_types.append("a1"+"q"+str(quant_bit))
                        accelerator_alloc[layer]="a1"+"q"+str(quant_bit)
                    else:
...
def design_choice_gen_apq(cifar): 
    if cifar:
        ...
        acc1_space={'comp_mode':[1],'trbuff':[42,21,7,6,3,1],'tcbuff':[42,21,7,6,3,1],'tmbuff':[16,8,4,2,1],'tnbuff':[16, 8,4,2,1], 'tr':[42,21,7,6,3,1],'tc':[42,21,7,6,3,1],'tm':[16,8,4,2,1],'tn':[16,8,4,2,1]}
        acc2_space={'comp_mode':[1],'trbuff':[21,7,3,1],'tcbuff':[21,7,3,1],'tmbuff':[32,16,8,4,2,1],'tnbuff':[32,16,8,4,2,1], 'tr':[21,7,3,1],'tc':[21,7,3,1],'tm':[32,16,8,4,2,1],'tn':[32,16,8,4,2,1]}
        acc3_space={'comp_mode':[1],'trbuff':[10,5,2,1],'tcbuff':[10,5,2,1],'tmbuff':[64,32,16,8,4,2,1],'tnbuff':[64,32,16,8,4,2,1], 'tr':[10,5,2,1],'tc':[10,5,2,1],'tm':[64,32,16,8,4,2,1],'tn':[64,32,16,8,4,2,1]}        
        dw_acc1_space={'comp_mode':[0,1],'trbuff':[42,21,7,6,3,1],'tcbuff':[42,21,7,6,3,1],'tmbuff':[16, 8,4,2,1],'tnbuff':[1], 'tr':[42,21,7,6,3,1],'tc':[42,21,7,6,3,1],'tm':[16,8,4,2,1],'tn':[1]}
        dw_acc2_space={'comp_mode':[0,1],'trbuff':[21,7,3,1],'tcbuff':[21,7,3,1],'tmbuff':[32,16,8,4,2,1],'tnbuff':[1], 'tr':[21,7,3,1],'tc':[21,7,3,1],'tm':[32,16,8,4,2,1],'tn':[1]}
        dw_acc3_space={'comp_mode':[0,1],'trbuff':[10,5,2,1],'tcbuff':[10,5,2,1],'tmbuff':[64,32,16,8,4,2,1],'tnbuff':[1], 'tr':[10,5,2,1],'tc':[10,5,2,1],'tm':[64,32,16,8,4,2,1],'tn':[1]}
