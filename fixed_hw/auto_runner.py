...
class auto_hls_run:
        ...
        for line in lines:
            ...
            if right_conv_mode and "partition anchor" in line:
                ...
                if pragma_info[2]==1:
                    ...
                    new_line+="#pragma HLS ARRAY_PARTITION variable=weight_temp cyclic factor=Tm dim=1\n"
                if pragma_info[3]==1:
                    new_line+="#pragma HLS ARRAY_PARTITION variable=feature_temp cyclic factor=Tn dim=1\n"
                    new_line+="#pragma HLS ARRAY_PARTITION variable=feature_temp1 cyclic factor=Tn dim=1\n"
                    new_line+="#pragma HLS ARRAY_PARTITION variable=weight_temp cyclic factor=Tn dim=2\n"
    ...
    def source_modifier(self, dnn_layer_all,dnn_layer_x,batch_id, batch_size=10):
        ...
        tr=sorted(self.r_factors(layer_struct[dnn_layer_x-1][2]),reverse=True)[0:-1]
        tc=sorted(self.r_factors(layer_struct[dnn_layer_x-1][2]),reverse=True)[0:-1]
        tm=sorted(self.r_factors(layer_struct[dnn_layer_x-1][1]),reverse=True)[0:-1]
        tn=sorted(self.r_factors(layer_struct[dnn_layer_x-1][0]),reverse=True)[0:-1]
        print(len(tr),len(tc),len(tm),len(tn))
        print(tm)
        print(tn)
        print(len(tr)*len(tc)*len(tm)*len(tn))
                                                        ...
                                                        for t_tnbuff in tn:
                                                            for t_tr in sorted(self.r_factors(t_trbuff),reverse=True)[0:-1]:
                                                                for t_tc in sorted(self.r_factors(t_tcbuff),reverse=True)[0:-1]:
                                                                    ...
                                                                    #option_pool.append(([t_tr,t_tc,t_tm,t_tn],t_cm,[1,1,1,1],[1,kernel_unroll_row,kernel_unroll_col]))
        print('dataset size', len(option_pool))
        ...
        for i in range(0,batch_size):
            ...
            print('starting_id',batch_id*batch_size+i)
...
for batch_id in range(0,10):
    ...
    os.system("rm -r src*")
    os.system("rm *.log")
os.system("mv results_pack* "+src+"layer"+str(dnn_layer_x))
    #move the npy
