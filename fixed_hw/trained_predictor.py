...
if dataset_size_check:
    exit()
np.random.shuffle(raw_data)
td_sz= int(train_data_ratio*len(raw_data))
train_data=raw_data[0:td_sz,:]
train_x=np.array([np.array(i) for i in train_data[:,0]],dtype='float32')[:,5:16]
print(train_x.shape)
...
train_loader=DataLoader(train_set,batch_size=int(bs),num_workers=29)


test_x=np.array([np.array(i) for i in test_data[:,0]],dtype='float32')[:,5:16]
...
optimizer = torch.optim.SGD(net.parameters(), lr=initial_lr,weight_decay=weight_decay)
...
for epoch in pbar:  # loop over the dataset multiple times
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if i % 1 == 0:    
            ...
            if data_normalize:
                ...
                for g in optimizer.param_groups:
                    print('current_learning_rate: ', g['lr'], end=' ')
                print(' ')
            else:
    ...
    if data_normalize:
    	...
    	pbar.write(str(torch.mean(torch.true_divide(torch.abs(tested_outputs-test_y),test_y)).item()))
    test_loss=error_loss(tested_outputs, test_y, mean_train_y[0],std_train_y[0])
    pbar.write('test loss: %.3f' % (test_loss.item()))
    print('predicted output mean: ', torch.mean(tested_outputs).item())
    print('label mean: ', torch.mean(test_y).item())
    #print(torch.true_divide(torch.abs((tested_outputs*std_train_y[0]+mean_train_y[0])-(test_y*std_train_y[0]+mean_train_y[0])),(test_y*std_train_y[0]+mean_train_y[0])).data)
    #scheduler.step()
